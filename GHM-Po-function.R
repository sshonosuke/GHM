...
GHM.Po <- function(Y, X, ID, G=4, L=3){
  maxitr <- 500
  delta <- 0.01
  m <- length( unique(ID) )
  N <- length(Y)
  ...
  for(i in 1:m){
    init.fit <- flexmix(Y[ID==i]~X[ID==i,], k=L, model=FLXMRglm(family="poisson"))
  ...
  for(k in 1:maxitr){
    ...
    for(k in 1:L){
      ww <- weight[,k]
      if( sum(ww)>5 ){
        fit <- glm(Y~X, weights=ww, family="poisson")
        hBeta[,k] <- coef(fit)
      }else{  hBeta[,k] <- hBeta0[,k]  }
    }
  ...
  Result <- list(ML=ML, Beta=hBeta, Pi=hPi, g=hg, BIC=BIC)
  return(Result)
}
...
###     adaptive GHM      ###
aGHM.Po <- function(Y, X, ID, rG=c(2,6), rL=c(2,5), print=F){
  ...
  for(g in 1:lG){
    dd <- list()
    for(l in 1:lL){
      dd[[l]] <- GHM.Po(Y, X, ID, G=sG[g], L=sL[l])
      BIC[g, l] <- dd[[l]]$BIC
      if(print){ 
